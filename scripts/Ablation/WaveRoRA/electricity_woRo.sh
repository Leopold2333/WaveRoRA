...
do
    ...
    log_file="logs/Ablation/${random_seed}/${random_seed}(${dataset_name})_${model}(${seq_len}-${pred_len})_el${e_layers}_wl${wavelet_layers}_${wavelet_type}.log"
    python $work_space $model --is_training=$is_training --gpu=$gpu \
    --num_workers=4 --seed=$random_seed --batch_size=$batch_size --loss=$loss \
    --seq_len=$seq_len --pred_len=$pred_len --enc_in=$enc_in --use_norm --embed_type=0 --residual --gate \
