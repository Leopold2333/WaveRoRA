...
import pandas as pd
import numpy as np
from data_provider.dataset import Dataset_Forecasting, Dataset_Forecasting_Solar
...
def load_data(args):
    data_type_param = {
        'train':    { 'shuffle_flag': True, 'drop_last': False, 'batch_size': args.batch_size },
    ...
    if args.dataset_name in ['weather', 'traffic', 'electricity', 'ETTh1', 'ETTh2', 'ETTm1', 'ETTm2']:
        df_raw = pd.read_csv(os.path.join(args.root_path, args.file_name))
        ...
        df_raw = []
        with open(os.path.join(args.root_path, args.file_name), "r", encoding='utf-8') as f:
